...
| Consolidated AI review   | **AI agent** | Single adversarial pass (`pnpm ai:verify`) covering React/Next.js, test quality, branch coverage, dead code/scope, business-logic placement, and correctness/safety. Runs both locally (`pnpm ai:verify`) and in CI as a dedicated step in `ai-agent-implement.yml`. Fixes critical/warning issues in-place; any resulting fixes are committed on the feature branch as a follow-up `chore(<IDENTIFIER>):` commit. The review summary (`/tmp/ai-review.md`) is posted as a PR comment. |
...
| AI review        | Automation | The review summary produced by `pnpm ai:verify` in Phase 3 is posted as a PR comment |
| Security scan    | Automation | CI runs CodeQL (SAST) and secretlint as separate workflows                           |
...
| Consolidated AI review   | `pnpm ai:verify` (Codex or Claude) | Local + CI (`ai-agent-implement.yml` step). Fixes critical/warning issues across React/Next.js, tests, branch coverage, dead code, business-logic placement, correctness/safety; summary posted to PR |
